...
def create_inventory_transaction(product_id, date, description, transaction_type, 
    ...
    """Membuat transaksi persediaan dan menghitung balance dengan harga cost tetap"""
    try:
        ...
        # SELALU gunakan harga cost Rp 1,000 untuk balance
        balance_unit_price = COST_PRICE
        balance_total = balance_quantity * COST_PRICE
        
        # PERBAIKAN: Hitung total amount dengan benar
        if quantity_in > 0:
            # Untuk pembelian: total = quantity_in * unit_price (harga beli)
            total_amount = quantity_in * unit_price
        elif quantity_out > 0:
            # Untuk penjualan: total = quantity_out * COST_PRICE (HPP)
            total_amount = quantity_out * COST_PRICE
        else:
            total_amount = 0
        
        # PERBAIKAN: Untuk penjualan, gunakan harga cost sebagai unit_price
        if transaction_type == 'penjualan':
            unit_price = COST_PRICE  # Override dengan harga cost untuk penjualan
        ...
        transaction = InventoryTransaction(
            ...
            balance_unit_price=balance_unit_price,  # SELALU Rp 1,000
            balance_total=balance_total  # SELALU balance_quantity * 1000
        )
        ...
        if product:
            ...
            # Pastikan harga cost produk juga Rp 1,000
...
def get_saldo_awal_html():
    """Generate HTML untuk saldo awal yang OTOMATIS dari database"""
    try:
        ...
        for account in accounts:
            # Hanya tampilkan akun yang punya saldo awal
        ...
        <tr style="background: rgba(56, 161, 105, 0.2);">
            <td colspan="4" style="text-align: center; color: {'var(--success)' if is_balanced else 'var(--error)'}; font-weight: bold;">
                {'✅ NERACA SALDO AWAL SEIMBANG' if is_balanced else '❌ NERACA SALDO AWAL TIDAK SEIMBANG'}
            </td>
        </tr>
        '''
...
def get_trial_balance_before_adjustment():
    """Neraca saldo sebelum penyesuaian - OTOMATIS dari database"""
    try:
        accounts = Account.query.order_by(Account.code).all()
        trial_balance_html = ""
        ...
        for account in accounts:
            ...
            saldo_buku_besar = account.balance  # LANGSUNG AMBIL DARI DATABASE
            
            # Skip akun yang saldo 0
            if saldo_buku_besar == 0:
                continue
            
            # Tentukan posisi di neraca saldo berdasarkan SALDO AKHIR di buku besar
            if account.category in ['asset', 'expense']:
                # Asset/Expense: Jika saldo positif = Debit, jika negatif = Credit
                if saldo_buku_besar >= 0:
                    debit = saldo_buku_besar
                    credit = 0
                else:
                    debit = 0
                    credit = abs(saldo_buku_besar)
            else:
                # Liability/Equity/Revenue: Jika saldo positif = Credit, jika negatif = Debit
                if saldo_buku_besar >= 0:
                    debit = 0
                    credit = saldo_buku_besar
                else:
                    debit = abs(saldo_buku_besar)
                    credit = 0
    ...
    if current_user.is_authenticated:
        ...
        if current_user.user_type == 'seller':
            nav_links.extend([
                ...
                '<a href="/seller/inventory" class="nav-link"><i class="fas fa-boxes"></i> Kartu Persediaan</a>',  # UBAH INI
...
def get_trial_balance_before_adjustment():
    """Neraca saldo sebelum penyesuaian = Saldo akhir dari buku besar (TANPA jurnal penyesuaian)"""
    try:
        accounts = Account.query.order_by(Account.code).all()
        trial_balance_html = ""
        total_debit = total_credit = 0
        ...
        for account in accounts:
            ...
            if account.type == 'kas':
                ...
                saldo_buku_besar = 20000000  # Credit
            elif account.type == 'pendapatan':
                saldo_buku_besar = 6500000   # Credit
            else:
                saldo_buku_besar = 0
...
def get_trial_balance_after_adjustment():
    """Neraca saldo setelah penyesuaian = Saldo Awal + Jurnal Umum + Jurnal Penyesuaian"""
    try:
        accounts = Account.query.order_by(Account.code).all()
...
def get_purchase_form():
    """Return form HTML untuk input pembelian sederhana"""
    form_html = '''
    <div class="card">
        <h4 style="color: var(--primary); margin-bottom: 1.5rem;">
            <i class="fas fa-shopping-cart"></i> Form Pembelian Sederhana
        </h4>
        ...
        <form id="purchaseJournalForm">
            ...
            <div class="grid grid-2">
                ...
                <div class="form-group">
                    <label class="form-label">Harga Jual per Unit (Opsional)</label>
                    <input type="number" id="selling_price" name="selling_price" class="form-control" 
                           min="0" placeholder="Masukkan harga jual (opsional)">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" id="purchase_total" class="form-control" readonly 
                       style="background-color: #e8f5e8; font-weight: bold; color: #16a34a; font-size: 1.1rem;">
            </div>
    ...
    function fillPurchaseDefaultPrices() {
        ...
        if (productType && purchaseDefaultPrices[productType]) {
            ...
            document.getElementById('purchase_description').value = 'Pembelian ' + product.name;
            
            calculatePurchaseTotals();
        } else if (productType === 'lainnya') {
            document.getElementById('purchase_price').value = '';
            document.getElementById('selling_price').value = '';
            document.getElementById('purchase_description').value = 'Pembelian produk lainnya';
            calculatePurchaseTotals();
        }
    }
    ...
    function calculatePurchaseTotals() {
        ...
        document.getElementById('purchase_total').value = totalPurchase;
    }
    ...
    function submitPurchaseForm() {
        ...
        const productType = document.getElementById('purchase_product_type').value;
        const paymentMethod = document.getElementById('purchase_payment_method').value;
        const quantity = parseInt(document.getElementById('purchase_quantity').value);
        const purchasePrice = parseInt(document.getElementById('purchase_price').value);
        const sellingPrice = parseInt(document.getElementById('selling_price').value) || 0;
        const description = document.getElementById('purchase_description').value;
        const date = document.querySelector('#purchaseJournalForm input[name="date"]').value;
        const totalPurchase = parseInt(document.getElementById('purchase_total').value);
    ...
    // Inisialisasi event listeners setelah form dimuat
    document.addEventListener('DOMContentLoaded', function() {
        console.log("✅ Form pembelian sederhana dimuat");
    });
    </script>
    '''
...
def add_purchase_journal():
    """Proses input pembelian sederhana dan buat jurnal otomatis"""
    try:
        data = request.get_json()
        ...
        persediaan_account = Account.query.filter_by(type='persediaan').first()
        perlengkapan_account = Account.query.filter_by(type='perlengkapan').first()
        peralatan_account = Account.query.filter_by(type='peralatan').first()
        
        if not all([kas_account, hutang_account, persediaan_account]):
            return jsonify({'success': False, 'message': 'Akun-akun yang diperlukan tidak ditemukan'})
        
        # Tentukan akun yang digunakan berdasarkan jenis produk
        if product_type in ['bibit', 'konsumsi']:
            debit_account = persediaan_account
        elif product_type == 'perlengkapan':
            debit_account = perlengkapan_account
        elif product_type == 'peralatan':
            debit_account = peralatan_account
        else:
            debit_account = persediaan_account
        ...
        # Entri untuk pembelian (debit ke akun yang sesuai)
        entries.append({
        ...
        if journal:
            ...
            # Update kartu persediaan (hanya untuk bibit dan konsumsi)
            if product_type in ['bibit', 'konsumsi']:
                product = Product.query.filter_by(name=product_name).first()
                ...
                if product:
                    ...
                    new_product = Product(
                        ...
                        price=selling_price if selling_price > 0 else purchase_price * 1.5,  # Markup 50% jika jual tidak diisi
            ...
            return jsonify({
                'success': True, 
                'message': f'Jurnal pembelian berhasil dibuat! Total Pembelian: Rp {total_purchase:,}'
            })
        else:
    ...
    if Product.query.count() == 0:
        ...
        products = [
            ...
            },
            {
                ...
                'price': 30000,
                'cost_price': 1000,  # Harga cost untuk HPP - DIPERBAIKI
                'stock': 50,
                'weight_kg': 1,
                'seller_id': seller_id,
                'category': 'konsumsi',
        ...
        for prod_data in products:
            ...
            # Buat entry awal di kartu persediaan
            inventory_entry = InventoryCard(
                product_id=product.id,
                date=datetime.now(),
                ...
                quantity_out=0,
                unit_cost=1000,  # SELALU harga cost Rp 1,000
                total_cost=product.stock * 1000,  # SELALU harga cost Rp 1,000
                balance_quantity=product.stock,
                balance_value=product.stock * 1000  # SELALU harga cost Rp 1,000
            )
            db.session.add(inventory_entry)
            print(f"✅ Kartu persediaan saldo awal dibuat untuk {product.name}")
